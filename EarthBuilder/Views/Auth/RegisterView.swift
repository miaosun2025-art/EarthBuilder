...
struct RegisterView: View {
    ...
    @State private var step: RegisterStep = .inputEmail
                ...
                ScrollView {
                    VStack(spacing: 30) {
                        ...
                        VStack(spacing: 16) {
                            ...
                            Text(step == .inputEmail ? "创建账号" : "验证邮箱")
                                .font(.system(size: 28, weight: .bold))
                                .foregroundColor(.white)

                            Text(step == .inputEmail ? "输入邮箱开始注册" : "请输入发送到邮箱的验证码")
                                .font(.system(size: 16))
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            TextField("", text: $email)
                .placeholder(when: email.isEmpty) {
                    Text("请输入邮箱").foregroundColor(.gray.opacity(0.5))
                }
    ...
    private func handleAction() {
        Task {
            ...
            case .inputCode:
                await authManager.verifyRegisterOTP(email: email, code: verificationCode)
                // 验证成功后，AuthManager 会自动设置 needsPasswordSetup = true
                // RootView 会检测到这个状态并显示设置密码页面
            }
        }
    }
}
